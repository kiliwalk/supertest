...
function PromiseEach(proms){
  var result = []
  return proms.reduce((chain, prom)=>{
    return chain.then(()=>prom).then(r=>{
      result.push(r);
    })
...
Test.prototype.assert = function(resError, res, fn) {
  ...
  // asserts
  PromiseEach(this._asserts.map(func=>this._assertFunction(func, res)))
  .then(()=>{
    // set unexpected superagent error if no other error has occurred.
    if (resError instanceof Error && (!res || resError.status !== res.status)) {
      throw resError;
    }
  }).then(()=>fn.call(this, null, res)).catch(err=>fn.call(this, err, res))
};
...
Test.prototype._assertFunction = function(check, res) {
  try{
    return Promise.resolve(check(res)).then(err=>{
